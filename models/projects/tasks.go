...
func QueryTaskDateSpan(tid int64, acceptid int64, v1 int64, v2 int64) (int64, int64, int) {
	var st, et int64
	var tt int
	var maps []orm.Params
	o := orm.NewOrm()
	sql := "select max(started) c1 from pms_projects_task where acceptid=? and started<=? and taskid != ?"
	num, _ := o.Raw(sql, acceptid, v1, tid).Values(&maps)
	if num > 0 {
		if maps[0]["c1"] != nil {
			foo := maps[0]["c1"].(string)
			st, _ = strconv.ParseInt(foo, 10, 64)
		} else {
			st = v1
		}
	}

	sql = "select min(ended) c2 from pms_projects_task where acceptid=? and ended>=? and taskid != ?"
	maps = nil
	num = 0
	num, _ = o.Raw(sql, acceptid, v2, tid).Values(&maps)
	if num > 0 {
		if maps[0]["c2"] != nil {
			foo := maps[0]["c2"].(string)
			et, _ = strconv.ParseInt(foo, 10, 64)
		} else {
			et = v2
		}
	}

	sql = "select sum(tasktime) c3 from pms_projects_task where acceptid=? and started >= ? and ended <= ? and taskid != ?"
	maps = nil
	num = 0
	num, _ = o.Raw(sql, acceptid, st, et, tid).Values(&maps)
	if num > 0 {
		if maps[0]["c3"] != nil {
			tt, _ = strconv.Atoi(maps[0]["c3"].(string))
		} else {
			tt = 0
		}
	}
