...
func (this *MyTaskProjectController) Get() {
	...
	condArr["taskassign"] = "0"
	if strings.Contains(this.GetSession("userPermission").(string), "task-assign") {
		condArr["taskassign"] = "taskassign"
	}
	condArr["taskbegin"] = "0"
	if strings.Contains(this.GetSession("userPermission").(string), "task-begin") {
		condArr["taskbegin"] = "taskbegin"
	}
	condArr["taskend"] = "0"
	if strings.Contains(this.GetSession("userPermission").(string), "task-end") {
		condArr["taskend"] = "taskend"
	}
	condArr["taskedit"] = "0"
	if strings.Contains(this.GetSession("userPermission").(string), "task-edit") {
		condArr["taskedit"] = "taskedit"
	}
...
func (this *TaskProjectController) Get() {
	...
	var acceptids int64
	acceptidtmp, _ := strconv.ParseInt(acceptid, 10, 64)
	...
	condArr["taskeditall"] = "0"
	if strings.Contains(this.GetSession("userPermission").(string), "task-editall") {
		condArr["taskeditall"] = "taskeditall"
	}
...
func (this *AddTaskProjectController) Get() {
	...
	this.Data["taskeditall"] = "0"
	if strings.Contains(this.GetSession("userPermission").(string), "task-editall") {
		this.Data["taskeditall"] = "taskeditall"
	}
...
func (this *AddTaskProjectController) Post() {
	...
	if !utils.CheckWorkTime(task.Tasktime, task.Started, task.Ended) {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "工时日期不匹配 日标准工时8"}
		this.ServeJSON()
		return
	}

	st, et, tt := QueryTaskDateSpan(task.Id, task.Acceptid, task.Started, task.Ended)
	logs.Warn("===QueryTaskDateSpan", st, et, tt)
	if !utils.CheckWorkTime(task.Tasktime+tt, st, et) {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "任务工时不匹配 请查看其他任务工时重叠"}
		this.ServeJSON()
		return
	}
	...
	if err == nil {
		...
		if acceptid != userid && strings.Contains(this.GetSession("userPermission").(string), "mail-notify") {
			...
			tshow := "<a href=" + link + ">" + link + "</a>"
			utils.SendEmail(acceptuser, "[GOWORK-TASK] "+task.Name, "<a>"+tshow+"</a>")
		}
	} else {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "任务添加失败"}
	}
...
func (this *EditTaskProjectController) Get() {
	...
	for i, v := range ccids {
		ccid, _ := strconv.ParseInt(v, 10, 64)
...
func (this *EditTaskProjectController) Post() {
	...
	st, et, tt := QueryTaskDateSpan(taskid, task.Acceptid, task.Started, task.Ended)
	logs.Warn("===QueryTaskDateSpan", st, et, tt)
	if !utils.CheckWorkTime(task.Tasktime+tt, st, et) {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "任务工时不匹配 请查看其他任务工时重叠"}
		this.ServeJSON()
...
func (this *DeleteTaskProjectController) Post() {
	//权限检测
	if !strings.Contains(this.GetSession("userPermission").(string), "task-delete") {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "无权设置"}
		this.ServeJSON()
		return
	}
	...
	if err == nil {
		this.Data["json"] = map[string]interface{}{"code": 1, "message": "删除任务成功", "projectid": prjid}
	} else {
